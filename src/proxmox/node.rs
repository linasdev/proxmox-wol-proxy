...
impl PwpProxmoxNode {
    pub fn new(settings: PwpSettings) -> Result<Self, PwpError> {
        let node_name = settings.proxmox.node_name;
        ...
        let proxmox_client = ProxmoxClient::builder(settings.proxmox.base_url.as_str())
            .api_token(
                settings.proxmox.token_id.as_str(),
                settings.proxmox.token_secret.as_str(),
            )
            .connect_timeout(Duration::from_millis(settings.proxmox.connect_timeout_ms))
            .accept_invalid_certs(settings.proxmox.accept_invalid_certs)
            .allow_insecure_http(settings.proxmox.allow_insecure_http)
            .build()?;

        info!(
            "Created Proxmox client with base URL: {}",
            settings.proxmox.base_url
        );
